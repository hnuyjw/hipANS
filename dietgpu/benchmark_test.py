...
def get_float_comp_timings(ts, num_runs=3):
    ...
    for i in range(1 + num_runs):
        ...
        comp = torch.zeros([rows, cols], dtype=torch.uint8, device=dev)
        ...
        out_ts = []
        for t in ts:
            out_ts.append(torch.zeros(t.size(), dtype=t.dtype, device=t.device))
...
for dt in [torch.bfloat16, torch.float16, torch.float32]:
    ...
    ts.append(torch.normal(0, 1.0, [3000], dtype=dt, device=dev))
    ...
    print("Float codec non-batched perf [66] {}".format(dt))
    print(
