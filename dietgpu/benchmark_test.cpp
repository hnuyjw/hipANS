...
std::tuple<size_t, size_t, double> calc_comp_ratio(
  ...
  // 检查compressed_sizes_tensor是否为一维张量，并且其长度与input_ts的元素数量相同
  ...
  // 确保compressed_sizes_tensor在CPU上，并且其数据类型为size_t
  ...
  for (size_t i = 0; i < input_ts.size(); ++i) {
    ...
    // 从compressed_sizes_tensor中获取压缩后的大小，并更新总压缩大小
...
std::vector<torch::Tensor> compressedMatrixToTensors(
  ...
  // auto stream = at::cuda::getCurrentCUDAStream();
	hipStream_t stream;
	CUDA_VERIFY(hipStreamCreate(&stream));
...
std::tuple<torch::Tensor, torch::Tensor, int64_t> compress_data_res(
  ...
  // std::cout<<"DietGpu.cpp:compress_data_res:maxOutputSize:"<<maxOutputSize<<"\n";
  //
  ...
  //std::cout<<"dietgpu/dietgpu/DietGpu.cpp:compress_data_res:comp:"<<comp[0][0]<<"\n";
  if (compressAsFloat) {
    auto config = FloatCompressConfig(
  	...
  	hipStream_t stream;
	  CUDA_VERIFY(hipStreamCreate(&stream));
    //std::cout<<"DietGpu.cpp:compPtrs.data():"<<compPtrs[0]<<"\n";
    //std::cout<<"DietGpu.cpp:comp指针:"<<&comp<<"\n";
    //std::cout<<"从DietGpu.cpp:compress_data_res函数进入floatCompress函数"<<"\n";
    floatCompress(
        res,
        config,
        tIns.size(),
...
std::tuple<double, double, size_t, size_t> get_float_comp_timings(const std::vector<torch::Tensor>& ts, int num_runs = 3) {  
    ...
    for (int i = 0; i < num_runs + 1; ++i) {    
        ...
        //auto [tensorCount, maxCompressedSize] = max_float_compressed_output_size(ts);    
        //rows = static_cast<int64_t>(tensorCount);
        //cols = maxCompressedSize;
...
std::tuple<float, float, size_t, size_t> get_any_comp_timings(const std::vector<torch::Tensor>& ts, int num_runs = 3) {  \
    torch::Device dev = torch::kCUDA;
    torch::Tensor tempMem = torch::empty({384 * 1024 * 1024}, torch::kUInt8).to(dev);
...
int main() {  
    ...
    torch::Device device(torch::kCUDA); // 或者torch::kCPU，取决于您的设备  
    ...
    for (torch::ScalarType dt : dtypes) {  
        ...
        ts.push_back(torch::randn({128 * 512 * 1024}, options)); // 使用randn代替normal，因为LibTorch没有normal函数  
  
        // 假设我们有一个get_float_comp_timings函数来获取压缩和解压缩的计时以及大小  
        double c, dc;  
        size_t total_size, comp_size;  
        std::tie(c, dc, total_size, comp_size) = get_float_comp_timings(ts);  
  
        // 打印信息（假设我们有了压缩和解压缩的计时和大小）  
    ...
    for (torch::ScalarType dt : dtypes) {  
        ...
        // 假设我们有一个get_any_comp_timings函数来获取压缩和解压缩的计时以及大小  
        double c, dc;  
        size_t total_size, comp_size;  
