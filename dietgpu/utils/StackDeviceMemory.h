...
struct GpuMemoryReservation {
  ...
  std::vector<T> copyToHost(hipStream_t stream) const {
    auto out = std::vector<T>(num);
    //std::cout<<num<<std::endl;
    CUDA_VERIFY(hipMemcpyAsync(
        out.data(), data(), num * sizeof(T), hipMemcpyDeviceToHost, stream));
...
class StackDeviceMemory {
 public:
  ...
  GpuMemoryReservation<T> alloc(
    ...
    auto allocPtr = allocPointer(stream, sizeToAlloc, type);
    //hipMemset(allocPtr, 0, sizeToAlloc);
