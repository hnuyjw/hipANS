...
#include "dietgpu/ans/BatchPrefixSum.h"
#include "dietgpu/ans/GpuANSCodec.h"
#include "dietgpu/ans/GpuANSStatistics.h"
...
__device__ void ansEncodeBlocksFull(
  ...
  int block =(blockIdx.x * blockDim.x + tid) / kWarpSize;
...
__device__ void ansEncodeCoalesce(
  ...
  if (tid < kWarpSize) {
    ...
    /*if(block>49000){
    printf("headerOut->getWarpStates()[block].warpState[tid]:%d\n",headerOut->getWarpStates()[block].warpState[tid]);
	    //printf("block: %d, tid:%d, headerOut->getWarpStates()[block].warpState[tid]:%d\n",block,tid,headerOut->getWarpStates()[block].warpState[tid]);
    }*/headerOut->getWarpStates()[block].warpState[tid] =
...
__global__ void ansEncodeCoalesceBatch(
  ...
  //if(blockIdx.x == 0)
  //printf("batch:%u, uncompressedWords:%u, numBlocks:%u\n ",batch,uncompressedWords,numBlocks);
...
void ansEncodeBatchDevice(
  ...
  //printf("numInBatch * maxNumCompressedBlocks * uncoalescedBlockStride:%d %lld\n",numInBatch * maxNumCompressedBlocks * uncoalescedBlockStride,numInBatch * maxNumCompressedBlocks * uncoalescedBlockStride);
  ...
  auto compressedWordsPrefix_dev =
      res.alloc<uint32_t>(stream, numInBatch * maxNumCompressedBlocks);
/*  uint32_t numElementsToPrint = numInBatch * maxNumCompressedBlocks * uncoalescedBlockStride;
  std::cout<<numElementsToPrint<<std::endl;
  std::vector<uint8_t> compressedBlocksHost(numElementsToPrint);
  //std::cout<<numElementsToPrint<<std::endl;
  // 从设备内存复制到主机内存
  hipMemcpy(compressedBlocksHost.data(), compressedBlocks_dev.data(),
             numElementsToPrint * sizeof(uint8_t), hipMemcpyDeviceToHost);
  // 打印数组内容
  std::cout << "Compressed Blocks:" << std::endl;
  for (size_t i = 0; i < compressedBlocksHost.size(); ++i) {
    std::cout << static_cast<int>(compressedBlocksHost[i]) << " ";
    if ((i + 1) % 128 == 0) std::cout << std::endl; // 每行打印 32 个值
  }
  std::cout << std::endl;
*/
...
#undef RUN_ENCODE
  ...
  for (size_t i = 0; i < compressedBlocksHost.size(); ++i) {
    std::cout << static_cast<int>(compressedBlocksHost[i]) << " ";
    if ((i + 1) % 32 == 0) std::cout << std::endl; // 每行打印 32 个值
  }
  std::cout << std::endl; // 打印换行以结束输出*/
